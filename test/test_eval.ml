...
let tests =
  [
    ...
    assertResult "M(q1 > x).Discard(q1) \\ ()"
      [
        [ ([ Complex.one; Complex.zero ], "Discard(q1)", 1.0); ];
      ];
    (* 2- Empty discard after a tau *)
    assertResult "Tau.Discard() \\ ()" [ [ ([Complex.one], "Discard()", 1.0) ] ];
    (* 3- Parallel after measurement *)
    assertResult "M(q1 > x).Discard(q1) || Discard() \\ ()"
      [
        [ ([ Complex.one; Complex.zero ], "Discard(q1) || Discard()", 1.0)];
      ];
    (* 4- Choice after measurement *)
    assertResult "(M(q1 > x).(Discard(q1) ++ c:quant!q1)) \\ ()"
      [
        [ ([ Complex.one; Complex.zero ], "c:quant!q1", 1.0)];
        [ ([ Complex.one; Complex.zero ], "Discard(q1)", 1.0)];
      ];
    (* 5- Choice with send *)
    ...
    (* 6- If after tau *)
    assertResult "Tau.if 0 = 0 then Discard() else c:int!5 \\ ()" [[([Complex.one], "Discard()", 1.0)]];
    (* 7- Parallel of choices *)
    assertResult "Tau.Discard(q2) ++ Tau.Discard(q2) || Tau.Discard(q1) ++ Tau.Discard(q1) \\ ()"
      [
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero], "Discard(q1) || Discard(q2)", 1.0)];
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero], "Discard(q1) || Discard(q2)", 1.0)];
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero], "Discard(q1) || Discard(q2)", 1.0)];
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero], "Discard(q1) || Discard(q2)", 1.0)];
      ];
    ...
    assertResult "M(q1 > x).c:quant!q1 || Tau.(if 0 = 0 then c1:int!1 else c1:int!2) \\ ()"
      [
        [([Complex.one; Complex.zero], "c:quant!q1 || c1:int!1", 1.0)]
      ];
    (* 10- Measure and parallel with if *)
    assertResult "M(q1 > x).(if x = 0 then Discard(q1, q2) else Discard(q2, q1)) || Tau.c:int!1 \\ ()"
      [
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero], "Discard(q1, q2) || c:int!1", 1.0)]
      ];
    (* 11- Triple parallel *)
    assertResult "Tau.Discard(q1) || Tau.Discard(q2) || Tau.Discard(q3) \\ ()"
      [
        [([Complex.one; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero], "Discard(q3) || Discard(q1) || Discard(q2)", 1.0)]
      ];
    ...
    assertResult "c:int!5 || c:int?x.Discard() \\ ()"
      [
        [([Complex.one], "Discard()", 1.0)]
      ];
    (* 14- Parallel with send, receive and expressions *)
    assertResult "c:int!(5+1) || c:int?x.if x = 6 then c:int!(x+1) else Discard() \\ ()"
      [
        [([Complex.one], "c:int!7", 1.0)]
      ];
    (* 15- QUANTUM LOTTERY *)
    assertResult "(H(q1).M(q1 > x).((if x = 0 then a:int!1 else b:int!1) || Discard(q1))) \\ ()"
      [
        [
          ([Complex.zero; Complex.one], "Discard(q1) || b:int!1", 0.5);
          ([Complex.one; Complex.zero], "Discard(q1) || a:int!1", 0.5);
        ]
      ];
    (* 16- TELEPORTATION (simplified) *)
    assertResult "(CX(q1,q2).H(q1).M(q1,q2 > n).(m:int!n || Discard(q1,q2))) || (m:int?n2.c:int!n2) \\ (m:int)"
      [
        [([Complex.zero; Complex.zero; Complex.one; Complex.zero], "c:int!1 || Discard(q1, q2)", 0.5);
         ([Complex.one; Complex.zero; Complex.zero; Complex.zero], "c:int!0 || Discard(q1, q2)", 0.5)]
      ];
    ...
    assertResult "(CX(q1,q2).H(q1).M(q1,q2 > n).(m:int!n || Discard(q1,q2))) || (m:int?n2.(if n2 = 0 then (I(q3).o:quant!q3) else (if n2 = 1 then (X(q3).o:quant!q3) else (if n2 = 2 then (Z(q3).o:quant!q3) else (Z(q3).X(q3).o:quant!q3))))) \\ (m:int)"
      [
        [([Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.one; Complex.zero; Complex.zero; Complex.zero], "X(q3).o:quant!q3 || Discard(q1, q2)", 0.5);
         ([Complex.one; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero; Complex.zero], "I(q3).o:quant!q3 || Discard(q1, q2)", 0.5)]
      ];
