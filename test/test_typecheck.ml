...
let tests = [
  ...
  assertException "Tau.(if (y=0) then a:int!1 else b:int!1) || Discard(q1) \\ ()";
  ...
  assertNotException "c1:quant?x.Discard(q1,x) || c2:quant?x.Discard(q2,x) \\ ()";
  ...
  assertNotException "c:quant?x.(c2:quant!x ++ Discard(x) ++ H(x).Discard(x)) \\ ()";
  (* 33- Sigma of all 3 processes is the same *)
  assertNotException "(c:quant?x.c2:quant!x ++ c:quant?x.Discard(x) ++ c:quant?x.H(x).Discard(x)) \\ ()";
  ...
  assertNotException "c1:quant?x.Discard(x) || c2:quant?x.Discard(x) \\ ()";
  (* 37- Choices share q1 but parallel don't *)
  assertNotException "Discard(q1) ++ Tau.(Discard(q1) || c:quant?x.(Discard(x))) \\ ()";
]
